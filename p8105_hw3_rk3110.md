...
`day_id` denotes the day of the week, `weekday` indicates if the day was
a weekday or not (1 for weekday, 0 for weekend), `minute` indicates the
minute of the day the observation occurs, starting at midnight, and
`activity` is the activity count for each observation.
